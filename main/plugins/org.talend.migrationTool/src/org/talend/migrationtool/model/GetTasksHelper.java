    ...
    public static List<MigrationTask> getMigrationTasks(final boolean beforeLogon) {
        ...
        ExtensionImplementationProvider<MigrationTask> provider = new ExtensionImplementationProvider<MigrationTask>(
            ...
            protected MigrationTask createImplementation(IExtension extension, IExtensionPointLimiter extensionPointLimiter,
                ...
                try {
                    if (Boolean.parseBoolean(configurationElement.getAttribute("beforeLogon")) == beforeLogon) { //$NON-NLS-1$
                        ...
                        // getMigrationTasks() api was used by old migration system. If lazy migration is enabled, need
                        // to exclude those lazy migrations from old system.
                        if (IMigrationToolService.isLazyMigraitonEnabled() && Boolean.parseBoolean(isLazy)) {
                            return null;
                        }
    ...
    public static List<IProjectMigrationTask> getProjectTasks(final Boolean beforeLogon, final boolean isLazy) {
        ...
        ExtensionImplementationProvider<IProjectMigrationTask> provider = new ExtensionImplementationProvider<IProjectMigrationTask>(
            ...
            protected IProjectMigrationTask createImplementation(IExtension extension,
                ...
                if (beforeLogon == null) {
                    ...
                    if (Boolean.parseBoolean(configurationElement.getAttribute("beforeLogon")) == beforeLogon.booleanValue()) { //$NON-NLS-1$
                        IProjectMigrationTask currentAction = create(configurationElement);
                        return currentAction;
                    }
                    return null;
        ...
        return provider.createInstances().stream().filter(t -> {
            if (IMigrationToolService.isLazyMigraitonEnabled()) {
                ...
                if(isLazy) {
                    // lazy migration is disabled, no lazy tasks
                    return false;
                }
                return true;
            }
