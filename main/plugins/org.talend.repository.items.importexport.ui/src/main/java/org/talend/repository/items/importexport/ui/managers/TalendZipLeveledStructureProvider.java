...
/**
 * This class is used instead of ZipLeveledStructureProvider where a bug exists
 * https://bugs.eclipse.org/bugs/show_bug.cgi?id=544417
 */
public class TalendZipLeveledStructureProvider implements ILeveledImportStructureProvider {
     ...
     */
    public TalendZipLeveledStructureProvider(ZipFile sourceFile) {
        super();
        zipFile = sourceFile;
        stripLevel = 0;
    }
    ...
    /**
     * Creates a new container zip entry with the specified name, iff it has not already been created. If the parent of
     * the given element does not already exist it will be recursively created as well.
     * 
    ...
    public boolean closeArchive() {
        try {
            ...
            IDEWorkbenchPlugin.log(DataTransferMessages.ZipImport_couldNotClose + getZipFile().getName(), e);
            return false;
        }
        return true;
    }
